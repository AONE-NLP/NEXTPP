...
from tpp.config_factory.config import Config
from tpp.config_factory.data_config import DataConfig
from tpp.config_factory.model_config import TrainerConfig, ModelConfig, BaseConfig
from tpp.utils import create_folder, logger, get_unique_id, get_stage, RunnerPhase, \
    MetricsHelper, DefaultRunnerConfig, py_assert, is_torch_available, \
    is_torch_gpu_available
from tpp.utils.const import Backend
...
class RunnerConfig(Config):
    ...
    def update_config(self):
        ...
        print("chunk_ratio:     ",self.model_config.chunk_ratio)
        if(self.base_config.model_id == "ODE_ENCODER"):
            model_folder_name = self.base_config.model_id +"_" +self.base_config.dataset_id + "_" +self.base_config.stage + "_"+str(self.model_config.chunk_ratio)
        else:    
            model_folder_name = self.base_config.model_id +"_" +self.base_config.dataset_id + "_" +self.base_config.stage
